...
class CustomAgent(BasicAgent):
    ...
    def __init__(self, vehicle, behavior='normal', opt_dict={}, map_inst=None, grp_inst=None):
        ...
        self.model = YOLO("model/yolo_fine_tuned.pt")  # Charger le modèle YOLO
    ...
    def _get_camera_image(self):
        """
        Retrieve the current frame from the camera sensor.
        """
        if hasattr(self, "sensor") and self.sensor is not None:
            image = self.sensor.get_image()  # Méthode d'acquisition propre à votre simulation
            return image
        return None
    ...
    def _affected_by_traffic_light(self, lights_list=None, max_distance=None):
        ...
        # Capture the image with the current camera
        camera_image = self._get_camera_image()
        if camera_image is None:
            return(False, None)
        
        # Use the YOLO model to detect traffic lights
        detections = self.model(camera_image, verbose=False)
        red_light_detected = any(
            det["name"] == "red_light" for det in detections[0].boxes.data
            )
    ...
    def _vehicle_obstacle_detected(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        """
        Detect obstacles (vehicles) in front of the agent using YOLO detection model.
        """
        if self._ignore_vehicles:
            return (False, None, -1)

        # Capture l'image actuelle de la caméra
        camera_image = self._get_camera_image()  # Fonction pour récupérer l'image
        if camera_image is None:
            return (False, None, -1)

        # Utiliser YOLO pour détecter les véhicules
        detections = self.model(camera_image, verbose=False)
        detected_vehicles = [
            det for det in detections[0].boxes.data if det["name"] == "vehicle"
        ]

        # Parcourir les véhicules détectés pour estimer la distance
        ego_transform = self._vehicle.get_transform()
        ego_location = ego_transform.location

        for vehicle in detected_vehicles:
            # Calculer la distance entre l'agent et l'obstacle détecté
            bbox_location = self._estimate_location(vehicle)
            distance = compute_distance(ego_location, bbox_location)
